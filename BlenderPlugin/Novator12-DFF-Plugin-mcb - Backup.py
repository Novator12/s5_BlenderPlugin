...
def read_rigid_geometry(js_geometry, js_clump, arm_o, frameIndex, frameRestMatrix, boneName, use_connect):
    ...
    print("Länge JS-Geometry TextureCoordinates-Table: " + str(len(js_geometry["textureCoordinates"])))
    
    if js_geometry["textureCoordinates"]:
    ...
    vertex_index = 0;
    if js_geometry["morphTargets"][0]["vertices"]:
...
class DynamicBoneListUIList(UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            layout.prop(item, "bone_type", text="")
