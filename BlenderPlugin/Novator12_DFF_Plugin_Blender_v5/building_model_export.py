...
from .Comfort.bin_mesh_utils import empty_bin_mesh, validate_bin_mesh
from .Comfort.constants import SPHERE_EXPORT_CENTER_PROP, SPHERE_EXPORT_RADIUS_PROP
from .Comfort.geometry_materials import sync_geometry_tool_materials
...
DEFAULT_PARTICLE_TEXTURE_PADDING = {
    "TexPadding": [0],
    "TextureAlphaPadding": [0, 98, 95, 97],
}
DEFAULT_PARTICLE_SIZE_SERI_MISSTAKE = -2130706433
...
def determine_bone_names(armature_object):
    if armature_object is None or armature_object.type != "ARMATURE":
        raise RuntimeError("Building export requires an Armature object.")

    bone_names = [bone.name for bone in armature_object.data.bones]
    if not bone_names:
        raise RuntimeError(f"Armature '{armature_object.name}' has no bones.")
    return bone_names
...
def _collect_bounding_sphere(mesh_object):
    for child in mesh_object.children:
        if child.type == "MESH" and child.data and child.data.name.startswith("Sphere"):
            sphere = OrderedDict()
            export_center = child.get(SPHERE_EXPORT_CENTER_PROP)
            export_radius = child.get(SPHERE_EXPORT_RADIUS_PROP)
            if export_center is not None and len(export_center) == 3 and export_radius is not None:
                sphere["x"] = export_center[0]
                sphere["y"] = export_center[1]
                sphere["z"] = export_center[2]
                sphere["radius"] = export_radius
            else:
...
def _build_bin_mesh_extension(metadata_entry):
    ...
    if raw_bin_mesh and raw_bin_mesh not in {"No data", "Empty-Geometry"}:
        try:
            ...
            return {
                "Flags": parsed.get("Flags", empty_bin_mesh()["Flags"]),
                "Meshes": parsed.get("Meshes", []),
            }
        except json.JSONDecodeError:
            print("[WARN] Invalid bin_mesh_data JSON, using default BinMeshPLG.")

    return empty_bin_mesh()


def _collect_triangles(mesh_object, strict=True):
    triangles = []
    for polygon in mesh_object.data.polygons:
        if strict and len(polygon.vertices) != 3:
            raise ValueError(
                f"Mesh '{mesh_object.name}' is not triangulated; face {polygon.index} has {len(polygon.vertices)} vertices."
            )
        triangle = OrderedDict()
        triangle["v1"] = polygon.vertices[0]
        triangle["v2"] = polygon.vertices[1]
        triangle["v3"] = polygon.vertices[2]
        triangle["materialId"] = int(polygon.material_index)
...
def build_building_geometry_payload(
    mesh_object,
    inverse_rest_matrix,
    geometry_metadata,
    strict_mesh_triangles=True,
    include_texture_coordinates=True,
):
    ...
    if include_texture_coordinates:
    ...
    payload["triangles"] = _collect_triangles(mesh_object, strict=strict_mesh_triangles)
    payload["materials"] = _build_material_payloads(mesh_object, metadata_entry)
    bin_mesh = _build_bin_mesh_extension(metadata_entry)
    bin_mesh_report = validate_bin_mesh(mesh_object, bin_mesh, len(payload["materials"]))
    if not bin_mesh_report["valid"]:
        stored_bin_mesh = _entry_value(metadata_entry, "bin_mesh_data", "No data")
        if stored_bin_mesh not in {None, "", "No data"}:
            print(
                f"[WARN] BinMesh for '{mesh_object.name}' is invalid and will be regenerated by S5Converter: "
                + "; ".join(bin_mesh_report["errors"][:3])
            )
        bin_mesh = empty_bin_mesh()
    payload["extension"] = {"BinMeshPLG": bin_mesh}
    return payload
...
def _geometry_payload_has_material_fx(geometry_payload):
    if not isinstance(geometry_payload, dict):
        return False

    for material in geometry_payload.get("materials", []):
        extension = material.get("extension", {})
        if "MaterialFXMat" in extension or "MaterialUVAnim" in extension:
            return True

    return False


def _normalize_particle_standard_payload(payload):
    if not isinstance(payload, dict):
        return {}

    normalized = deepcopy(payload.get("ParticleStandard", payload))
    emitters = normalized.get("Emitters")
    ...
    for emitter in emitters:
        ...
        emitter_standard = emitter.get("EmitterStandard")
        if not isinstance(emitter_standard, dict):
            continue

        legacy_particle_size = emitter_standard.pop("ParticleSize_SeriMisstake", None)
        if legacy_particle_size is not None:
            emitter_standard["ParticleSizeSeriMisstake"] = legacy_particle_size
        elif "ParticleSize" in emitter_standard and "ParticleSizeSeriMisstake" not in emitter_standard:
            emitter_standard["ParticleSizeSeriMisstake"] = DEFAULT_PARTICLE_SIZE_SERI_MISSTAKE

        particle_texture = emitter_standard.get("ParticleTexture")
        if not isinstance(particle_texture, dict):
            continue

        particle_texture.setdefault("TexPadding", list(DEFAULT_PARTICLE_TEXTURE_PADDING["TexPadding"]))
        particle_texture.setdefault(
            "TextureAlphaPadding",
            list(DEFAULT_PARTICLE_TEXTURE_PADDING["TextureAlphaPadding"]),
        )
        particle_texture.setdefault("extension", {})

    return normalized


def build_building_atomic_entry(
    frame_index,
    geometry_index,
    particle_data,
    bone_type_data,
    ...
    particle_data_map,
    geometry_has_material_fx=False,
):
    ...
    if particle_data:
        for effect_entry in particle_data:
            ...
            if frame_index in particle_data_map:
                atomic_entry["extension"] = {
                    "ParticleStandard": _normalize_particle_standard_payload(particle_data_map[frame_index])
                }
            elif effect_key in PARTICLE_EFFECT_LUT:
                atomic_entry["extension"] = {
                    "ParticleStandard": _normalize_particle_standard_payload(PARTICLE_EFFECT_LUT[effect_key])
                }
            break
...
def _activate_armature_for_export(context, armature_object):
    ...
    if context.view_layer.objects.active != armature_object:
        raise RuntimeError(f"Could not activate Armature '{armature_object.name}'.")
...
def resolve_armature_for_export(context):
    ...
    if active_object is None:
        raise RuntimeError(
            "No Armature object is available in the active View Layer."
        )

    _activate_armature_for_export(context, active_object)
    return active_object


def collect_armature_export_state(armature_object):
    bone_names_sorted = determine_bone_names_sorted(armature_object)
    _activate_armature_for_export(bpy.context, armature_object)

    if not bpy.ops.object.mode_set.poll():
        raise RuntimeError(
            f"Cannot enter Edit Mode for Armature '{armature_object.name}'."
        )
    ...
    try:
        if armature_object.mode != "EDIT":
            raise RuntimeError(
                f"Armature '{armature_object.name}' did not enter Edit Mode."
            )
        if len(armature_object.data.edit_bones) != len(bone_names_sorted):
            raise RuntimeError(
                f"Armature '{armature_object.name}' exposes "
                f"{len(armature_object.data.edit_bones)} edit bones for "
                f"{len(bone_names_sorted)} bones."
            )
        ...
        if armature_object.mode == "EDIT" and bpy.ops.object.mode_set.poll():
            bpy.ops.object.mode_set(mode="OBJECT")
...
def build_building_export_json(
    ...
    atomic_material_fx_data,
    particle_data_map,
    geometry_source_indices=None,
    strict_mesh_triangles=True,
    geometry_target_names=None,
    include_geometry_texture_coordinates=True,
):
    ...
    target_names = set(geometry_target_names) if geometry_target_names is not None else None
    for target in collect_geometry_export_targets(scene, armature_object):
        if target_names is not None:
            if target["kind"] != "mesh" or target["mesh_object"].name not in target_names:
                continue
        ...
        if target["kind"] == "mesh":
            ...
            clump["geometries"].append(
                build_building_geometry_payload(
                    mesh_object,
                    frame_rest_matrix.inverted(),
                    geometry_data,
                    strict_mesh_triangles=strict_mesh_triangles,
                    include_texture_coordinates=include_geometry_texture_coordinates,
                )
            )
            if geometry_source_indices is not None:
                geometry_source_indices[mesh_object.name] = geometry_index
        else:
            entry = target["entry"]
        ...
        geometry_payload = clump["geometries"][geometry_index]
        ...
        clump["atomics"].append(
            build_building_atomic_entry(
                ...
                atomic_material_fx_data,
                particle_data_map,
                geometry_has_material_fx=_geometry_payload_has_material_fx(geometry_payload),
            )
        )
    payload = OrderedDict()
...
class BuildingExportOperator(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        try:
            material_sync = sync_geometry_tool_materials(context.scene)
        except Exception as exc:
            message = f"Material synchronization failed: {exc}"
            print(f"[Geometry Materials] ERROR: {message}")
            self.report({"ERROR"}, message)
            return {"CANCELLED"}

        if material_sync["errors"]:
            for message in material_sync["errors"]:
                print(f"[Geometry Materials] ERROR: {message}")
            self.report(
                {"ERROR"},
                f"Material synchronization failed for {len(material_sync['errors'])} Geometry entries. "
                "See the Blender Info Log.",
            )
            return {"CANCELLED"}

        for changed_entry in material_sync["changed_entries"]:
            for change in changed_entry["changes"]:
                print(
                    f"[Geometry Materials] Geometry {changed_entry['index'] + 1} "
                    f"({changed_entry['name']}): {change}"
                )
        ...
        try:
            ...
            if material_sync["changed_count"]:
                self.report(
                    {"INFO"},
                    f"Export completed; synchronized materials for "
                    f"{material_sync['changed_count']} Geometry entries.",
                )
            else:
                self.report({"INFO"}, "Export completed; Geometry materials already matched the Scene meshes.")
            return {"FINISHED"}
        except Exception as exc:
